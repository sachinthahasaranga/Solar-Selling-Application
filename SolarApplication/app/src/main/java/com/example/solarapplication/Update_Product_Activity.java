...
public class Update_Product_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      //  account = (Suppliers) intent.getSerializableExtra("UserInfo");
        post = (Products) intent.getSerializableExtra("PostData");
    ...
    name.setText(post.getProductName());
    brand.setText(post.getBrandName());
    price.setText(post.getPrice());
    warrantyperiod.setText(post.getWarrantyPeriod());
    description.setText(post.getDescription());
        ...
        String selectedCategory = post.getCategory(); // The category you want to set as selected
        int position = adapter.getPosition(selectedCategory);
        SpinnerCategoryName.setSelection(position);

        byte[] imageByteArray = post.getImage();
        Bitmap bitmap = BitmapFactory.decodeByteArray(imageByteArray, 0, imageByteArray.length);
        image.setImageBitmap(bitmap);

        int UserId=post.getSupplierId();
        // Get Data From Supplier Table Where Id equals to UserId
        // and pass that account data to View_Supplier_Products_Activity

        Suppliers supplier = dbh.getSupplierById(UserId);

        if (supplier != null) {
        } else {
            Intent intent2 = new Intent(Update_Product_Activity.this, Login_Activity.class);
            Toast.makeText(Update_Product_Activity.this, "Supplier data not found", Toast.LENGTH_SHORT).show();
        }
      ...
      Back.setOnClickListener(new View.OnClickListener() {
        ...
        public void onClick(View view) {
                Intent intent = new Intent(Update_Product_Activity.this, View_Supplier_Products_Activity.class);
                intent.putExtra("UserInfo", supplier);
        ...
        public void onClick(View view) {
            ...
            if (ProductName.isEmpty()){
                ...
                showError(name, "Toy Name is In Valid");
                ...
                AlertDialog.Builder alertDialog = new AlertDialog.Builder(Update_Product_Activity.this);
                alertDialog.setTitle("Confirm...");
                alertDialog.setMessage("Are you sure you want to Update data");
                        ...
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                               ...
                               byte[] img = imageViewToByte(image);
                                Boolean insert = dbh.updateProductData(post.getId(),img, ProductName, BrandName, Price, Description,Category, WarrantyPeriod,"Pending");
                                if (insert == true) {
                                    Toast.makeText(Update_Product_Activity.this, "Post Updated Successfully", Toast.LENGTH_SHORT).show();
                                    Intent intent = new Intent(Update_Product_Activity.this, View_Supplier_Products_Activity.class);
                                    intent.putExtra("UserInfo", supplier);
                                    startActivity(intent);
                                    overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
                                } else {
                                    Toast.makeText(getApplicationContext(), "Failed to update product post", Toast.LENGTH_LONG).show();
                                }
